...
/* Lançamentos "fixos": ao abrir o mês atual, copia os recorrentes dos últimos
   3 meses que ainda não existem neste mês (idempotente). */
function ensureRecurring(userId) {
  const month = currentMonth();
  const nowIdx = monthIndex(month + '-01');
  const from = indexToMonth(nowIdx - 3);
  const templates = db.prepare(`
    SELECT type, description, category, amount_cents, date
    FROM transactions
    WHERE user_id = ? AND recurring = 1
      AND substr(date, 1, 7) >= ? AND substr(date, 1, 7) < ?
    ORDER BY date ASC
  `).all(userId, from, month);
  if (!templates.length) return;

  // mantém só a ocorrência mais recente de cada (tipo, descrição)
  const latest = new Map();
  for (const t of templates) latest.set(`${t.type}|${t.description.toLowerCase()}`, t);

  const existing = new Set(db.prepare(`
    SELECT type, description FROM transactions
    WHERE user_id = ? AND substr(date, 1, 7) = ?
  `).all(userId, month).map((e) => `${e.type}|${e.description.toLowerCase()}`));
  ...
  for (const [key, t] of latest) {
    ...
    const day = Math.min(Number(t.date.slice(8, 10)), 28);
    ins.run(userId, t.type, t.description, t.category, t.amount_cents,
...
function buildProjection(userId) {
  ...
  // "Total restante" desconta a parcela deste mês: ela já está na fatura atual
  // (a vencer), então o saldo devedor real só conta o que vem DEPOIS dela —
  // é assim que a fatura do cartão de crédito costuma apresentar o total.
  const currentMo = currentMonth();
  const totalDebt = installments.reduce((s, i) => {
    const dueThisMonth = i.next_due_month === currentMo ? i.installment_cents : 0;
    return s + i.remaining_cents - dueThisMonth;
  }, 0);
...
async function handleApi(req, res, url) {
  ...
  if (method === 'GET' && pathname === '/api/transactions') {
    ...
    if (month === currentMonth()) ensureRecurring(user.id);
  ...
  if (m && method === 'PUT') {
    ...
    db.prepare(`
      UPDATE transactions
      SET type = ?, description = ?, category = ?, amount_cents = ?, date = ?, recurring = ?
      WHERE id = ? AND user_id = ?
    `).run(type, description, category, amount, date, b.recurring ? 1 : 0, Number(m[1]), user.id);
    return json(res, 200, { ok: true });
  }
  ...
  if (method === 'GET' && pathname === '/api/market') {
    try {
      return json(res, 200, await getMarket());
    } catch {
      return json(res, 502, { error: 'Não foi possível consultar as cotações agora.' });
    }
  }
  ...
  if (method === 'GET' && pathname === '/api/goals') {
    const rows = db.prepare(`
      SELECT id, name, target_cents, saved_cents, target_date, created_at
      FROM goals WHERE user_id = ? ORDER BY (saved_cents >= target_cents), id DESC
    `).all(user.id);
  ...
  if (method === 'POST' && pathname === '/api/goals') {
    ...
    const target = toCents(b.target_cents);
    const targetDate = cleanText(b.target_date, 10);
    if (!name || !target) return json(res, 400, { error: 'Dê um nome e um valor para a meta.' });
    if (targetDate && !isValidDate(targetDate)) return json(res, 400, { error: 'Data alvo inválida.' });
    const info = db.prepare(`
      INSERT INTO goals (user_id, name, target_cents, target_date) VALUES (?, ?, ?, ?)
    `).run(user.id, name, target, targetDate || null);
    return json(res, 201, { id: Number(info.lastInsertRowid) });
  }

  m = pathname.match(/^\/api\/goals\/(\d+)\/add$/);
  if (m && method === 'POST') {
    const b = await readBody(req);
    const amount = Math.round(Number(b.amount_cents));
    if (!Number.isFinite(amount) || amount === 0) return json(res, 400, { error: 'Valor inválido.' });
    db.prepare(`
      UPDATE goals SET saved_cents = MAX(saved_cents + ?, 0) WHERE id = ? AND user_id = ?
    `).run(amount, Number(m[1]), user.id);
    return json(res, 200, { ok: true });
  }

  m = pathname.match(/^\/api\/goals\/(\d+)$/);
  if (m && method === 'DELETE') {
    db.prepare('DELETE FROM goals WHERE id = ? AND user_id = ?').run(Number(m[1]), user.id);
    return json(res, 200, { ok: true });
  }
  ...
  if (method === 'GET' && pathname === '/api/budgets') {
    const month = currentMonth();
    const rows = db.prepare('SELECT id, category, limit_cents FROM budgets WHERE user_id = ? ORDER BY category').all(user.id);
    const spent = db.prepare(`
      ...
      GROUP BY category
    `).all(user.id, month);
    const byCat = new Map(spent.map((s) => [s.category.toLowerCase(), Number(s.total)]));
    return json(res, 200, {
      month,
      items: rows.map((r) => ({ ...r, spent_cents: byCat.get(r.category.toLowerCase()) ?? 0 })),
    });
  }
  ...
  if (method === 'POST' && pathname === '/api/budgets') {
    ...
    const limit = toCents(b.limit_cents);
    if (!category || !limit) return json(res, 400, { error: 'Informe a categoria e o limite mensal.' });
    db.prepare(`
    ...
    `).run(user.id, category, limit);
    return json(res, 201, { ok: true });
  }

  m = pathname.match(/^\/api\/budgets\/(\d+)$/);
  if (m && method === 'DELETE') {
    db.prepare('DELETE FROM budgets WHERE id = ? AND user_id = ?').run(Number(m[1]), user.id);
    return json(res, 200, { ok: true });
  }

  /* --- backup e restauração --- */
  if (method === 'GET' && pathname === '/api/backup') {
    const pick = (table) => db.prepare(`SELECT * FROM ${table} WHERE user_id = ?`).all(user.id)
    ...
    const payload = {
      ...
      installments: pick('installments'),
      debts: pick('debts'),
      goals: pick('goals'),
    ...
    res.writeHead(200, {
      ...
      'Content-Disposition': `attachment; filename="backup-meu-financeiro-${new Date().toISOString().slice(0, 10)}.json"`,
    });
    return res.end(JSON.stringify(payload, null, 1));
  }

  if (method === 'POST' && pathname === '/api/restore') {
    const b = await readBody(req);
    if (b.app !== 'meu-financeiro' || !Array.isArray(b.transactions)) {
      return json(res, 400, { error: 'Arquivo de backup inválido.' });
    }
    db.exec('BEGIN');
    try {
      for (const t of ['transactions', 'installments', 'debts', 'goals', 'budgets']) {
        db.prepare(`DELETE FROM ${t} WHERE user_id = ?`).run(user.id);
      }
      const insTx = db.prepare('INSERT INTO transactions (user_id, type, description, category, amount_cents, date, recurring) VALUES (?, ?, ?, ?, ?, ?, ?)');
      for (const r of b.transactions) insTx.run(user.id, r.type, r.description, r.category ?? 'Outros', r.amount_cents, r.date, r.recurring ? 1 : 0);
      const insInst = db.prepare('INSERT INTO installments (user_id, description, category, installment_cents, total_installments, paid_installments, first_due_date) VALUES (?, ?, ?, ?, ?, ?, ?)');
      for (const r of (b.installments ?? [])) insInst.run(user.id, r.description, r.category ?? 'Outros', r.installment_cents, r.total_installments, r.paid_installments ?? 0, r.first_due_date);
      const insDebt = db.prepare('INSERT INTO debts (user_id, direction, person, description, amount_cents, due_date, paid) VALUES (?, ?, ?, ?, ?, ?, ?)');
      for (const r of (b.debts ?? [])) insDebt.run(user.id, r.direction, r.person, r.description ?? '', r.amount_cents, r.due_date ?? null, r.paid ? 1 : 0);
      const insGoal = db.prepare('INSERT INTO goals (user_id, name, target_cents, saved_cents, target_date) VALUES (?, ?, ?, ?, ?)');
      for (const r of (b.goals ?? [])) insGoal.run(user.id, r.name, r.target_cents, r.saved_cents ?? 0, r.target_date ?? null);
      const insBud = db.prepare('INSERT INTO budgets (user_id, category, limit_cents) VALUES (?, ?, ?)');
      for (const r of (b.budgets ?? [])) insBud.run(user.id, r.category, r.limit_cents);
      db.exec('COMMIT');
    } catch (err) {
      db.exec('ROLLBACK');
      return json(res, 400, { error: 'Backup com dados inconsistentes — nada foi alterado.' });
    }
    return json(res, 200, { ok: true });
  }
