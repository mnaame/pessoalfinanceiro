...
async function handleApi(req, res, url) {
  ...
  if (method === 'POST' && pathname === '/api/installments') {
    ...
    const firstDue = cleanText(b.first_due_date, 10);
    if (!description || !value || !Number.isInteger(total) || total < 1 || total > 480 || !isValidDate(firstDue)) {
      return json(res, 400, { error: 'Preencha descrição, valor da parcela, quantidade e 1º vencimento.' });
    }
